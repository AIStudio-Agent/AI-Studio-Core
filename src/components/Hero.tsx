...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const charsPerSecond = 20; // Adjust typing speed here (20 chars/sec = ~50ms/char)
  ...
  const [prefix, suffix] = typedText.includes('The ') 
    ? typedText.split('The ') 
    : ['', typedText];
          ...
          <div className="lg:w-1/2 lg:pr-12 mb-12 lg:mb-0">
            ...
            <h1 className="font-['Space_Grotesk'] text-5xl md:text-6xl lg:text-7xl font-bold leading-tight text-white mb-6 min-h-[1.2em]">
              {typedText.includes('The ') ? (
                <>
                  The{' '}
                  <span style={{ color: colors.accent[500] }}>
        ...
        <div className="mt-20 text-center">
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 opacity-70">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="h-8 w-24 bg-white bg-opacity-20 rounded"
              ></div>
            ))}
          </div>
        </div>
